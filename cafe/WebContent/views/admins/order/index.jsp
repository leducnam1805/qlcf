    ...
    <table class="table table-bordered" id="datatable">
	  ...
	  <a href="123">
 		<tr class="tr-hover">
	      ...
	      <td><%=objItem.getProduct().getMenu().getName()%></td>
	      <td><%=objItem.getPrice()%></td>
	      <td><%=objItem.getQuantity()%></td>
	      <td><%=objItem.getOrderProduct().getUserID().getName()%></td>
	      <td><%=objItem.getOrderProduct().getTotal()%></td>
	      <td><%=objItem.getCreateDate()%></td>
	     <td>
	    	<button type="button" class="btn btn-warning suaMenu">
				<a href="<%=request.getContextPath()%>/admin/order/cat/edit?id=<%=objItem.getId()%>">Xem chi tiết</a>
			</button>
	     </td>
	    </tr>
	   </a>
